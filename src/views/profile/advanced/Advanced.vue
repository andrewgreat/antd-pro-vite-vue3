    ...
    >
      ...
      >
        <template v-slot:bodyCell="{ column, record }">
          <a-badge v-if="column.key === 'status'"  :status="statusTypeFilter(record.status)" :text="statusFilter(record.status)"/>
        </template>
      </a-table>
      <a-table
...
export default defineComponent({
  ...
  setup () {
    ...
    const operationTabList = reactive([
      {
        key: '1',
        tab: '操作日志一'
      },
    ...
    const operationActiveTabKey = ref('1')
    ...
    const statusTypeFilter = computed(() => {return (type) => {
      const statusTypeMap = {
        'agree': 'success',
        'reject': 'error'
      }
